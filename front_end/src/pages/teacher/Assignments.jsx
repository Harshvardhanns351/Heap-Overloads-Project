...
export default function TeacherAssignments() {
  ...
  const analytics = useMemo(() => {
    ...
    return {
      ...
      avgMarks: totals.avgMarksCount ? (totals.avgMarksTotal / totals.avgMarksCount).toFixed(1) : 'ù',
                ...
                {items.map((assignment) => {
                  ...
                  return (
                    <button key={assignment.id} type="button" onClick={() => openAssignment(assignment, 'view')} style={{ textAlign: 'left', padding: '14px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
                      ...
                      <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px' }}>{assignment.subject} ò {assignment.class_id}</div>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{summary.submissionRate}% submitted ò {summary.missingStudents.length} pending ò {summary.lateCount} late</div>
                    </button>
                  );
                })}
              </div>
                ...
                <div className="card" style={{ padding: '18px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '10px', marginBottom: '14px' }}>
                    ...
                    <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)' }}>
                      ...
                      <div style={{ fontSize: '16px', fontWeight: '700' }}>{selectedSummary.avgMarks !== null ? selectedSummary.avgMarks.toFixed(1) : 'ù'}</div>
                    </div>
                    <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)' }}>
