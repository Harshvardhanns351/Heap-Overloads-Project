      ...
      <div className="card" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>Administrator · {user?.email}</div>
          </div>
          {!edit
        ...
        <div style={{ display: 'grid', gap: '16px' }}>
          <div>
            ...
            {edit
              ...
              : <p style={{ fontSize: '13px', margin: 0, color: bio ? 'var(--text-primary)' : 'var(--text-muted)' }}>{bio || 'No bio.'}</p>
            }
          </div>
          <div>
          ...
          {[['Role', 'Admin'], ['Email', user?.email]].map(([label, val]) => (
            <div key={label}>
