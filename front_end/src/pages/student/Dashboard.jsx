...
import {
  ...
  Code2, ExternalLink, FileText, LayoutDashboard,
  Link2, Loader2, Paperclip, RefreshCw, Send, Star, Timer,
  TrendingUp, Trophy, Upload, Zap, ChevronDown, ChevronUp, MessageSquare
} from 'lucide-react';
...
function dueIn(deadline) {
  ...
  if (diff < 0) return { text: 'Overdue', color: '#f87171', urgent: true };
  if (diff === 0) return { text: 'Due today', color: '#fbbf24', urgent: true };
  if (diff === 1) return { text: 'Due tomorrow', color: '#fbbf24', urgent: true };
  return { text: `${diff}d left`, color: 'var(--text-muted)', urgent: false };
}
const safeH = v => (v != null && !isNaN(v) && v > 0) ? `${Number(v).toFixed(1)}h` : '0h';
const TIERS = [
  { min: 950, label: 'Legend', color: '#f87171' }, { min: 800, label: 'Elite', color: '#fbbf24' },
  { min: 600, label: 'Expert', color: '#a78bfa' }, { min: 400, label: 'Coder', color: '#60a5fa' },
  { min: 200, label: 'Learner', color: '#34d399' }, { min: 0, label: 'Beginner', color: '#94a3b8' },
];
const getTier = s => TIERS.find(t => s >= t.min) || TIERS[TIERS.length - 1];
const PLAT_COLOR = { leetcode: '#fbbf24', github: '#8b949e', codeforces: '#60a5fa', codechef: '#cd7f32' };

// ─── tiny shared pieces ───────────────────────────────────────────────────────
function SectionTabs({ tabs, active, onChange }) {
  return (
    <div style={{ display: 'flex', gap: '3px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '10px', padding: '4px', marginBottom: '22px', flexWrap: 'wrap' }}>
      {tabs.map(t => (
...
function OverviewSection({ currentUser, marks, riskScore, roadmapNodes, assignments, sprintStats, codingSummary, veloris }) {
  ...
  const completedNodes = roadmapNodes?.filter(n => n.status === 'completed' || n.status === 'complete').length || 0;
    ...
    <div>
      ...
      <div style={{ marginBottom: '22px' }}>
        ...
        <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
          {overdue > 0 && <span style={{ color: 'var(--status-err)' }}>⚠ {overdue} overdue assignment{overdue !== 1 ? 's' : ''}</span>}
          {pending > 0 && <span style={{ color: 'var(--status-warn)' }}>📋 {pending} pending</span>}
      ...
      {currentUser?.pending_nudge && (
        <div style={{ padding: '14px 18px', marginBottom: '24px', display: 'flex', alignItems: 'flex-start', gap: '12px', background: 'rgba(255,255,255,0.04)', borderRadius: '12px', borderLeft: '3px solid var(--accent)', border: '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        ...
        <StatCard label="Roadmap Progress" value={`${completedNodes}/${totalNodes}`} sub="Nodes completed" color="var(--text-muted)" icon={TrendingUp} />
        <StatCard label="Study Time" value={`${studyHours}h`} sub="This week (sprints + coding)" color="var(--text-muted)" icon={Timer} />
        <StatCard label="Problems Solved" value={totalProblems || "—"} sub="Across all platforms" color="var(--accent)" icon={Code2} />
      </div>
          ...
          {radarData.length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <RadarChart data={radarData}>
                ...
                <Radar dataKey="score" stroke="var(--accent)" fill="var(--accent)" fillOpacity={0.1} strokeWidth={1.5} dot={{ fill: 'var(--accent)', r: 3 }} />
              </RadarChart>
            </ResponsiveContainer>
          ) : (
              ...
              {Object.entries(subjectScores).map(([sub, score]) => (
                <div key={sub}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: '700', color: score < 60 ? 'var(--status-err)' : score < 75 ? 'var(--status-warn)' : 'var(--status-ok)' }}>{score}%</span>
                  </div>
                  <ProgressBar value={score} color={score < 60 ? 'var(--status-err)' : score < 75 ? 'var(--status-warn)' : 'var(--status-ok)'} height={4} />
                </div>
              ))}
            </div>
          ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          {assignments?.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {assignments.slice(0, 5).map(a => {
                ...
                return (
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: `1px solid ${due.urgent && sc === 'not_submitted' ? 'var(--status-err)' : 'var(--border)'}` }}>
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap ${sc === 'submitted' ? 'badge-green' : sc === 'late' ? 'badge-red' : 'badge-yellow'}`}>
                      {sc === 'not_submitted' ? 'pending' : sc}
                    </span>
                  </div>
                );
              })}
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '18px' }}>
          ...
          {codingSummary?.recent_submissions?.length > 0 ? (
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', textAlign: 'center', padding: '20px 0' }}> Link a coding platform to see activity </div>
          )}
        </div>
      </div>
    </div>
...
function SubmitModal({ assignment, isEdit = false, onClose, onSubmitted }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(520px,92vw)', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '16px', zIndex: 201, padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
          {[['text', 'Text', FileText], ['file', 'File', Upload]].map(([m, label, Icon]) => (
            <button key={m} onClick={() => setMode(m)} style={{ flex: 1, padding: '8px', borderRadius: '8px', fontSize: '12px', fontWeight: 500, cursor: 'pointer', border: `1.5px solid ${mode === m ? 'var(--accent)' : 'var(--border)'}`, background: mode === m ? 'var(--accent-bg)' : 'transparent', color: mode === m ? 'var(--accent)' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
              <Icon size={12} />{label}
            </button>
          ))}
        </div>
        {mode === 'text' && (
          <textarea className="input" value={text} onChange={e => setText(e.target.value)} placeholder="Write your answer here..." rows={5} style={{ resize: 'vertical' }} />
        )}
        {mode === 'file' && (
          <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed var(--border)', borderRadius: '10px', padding: '28px', textAlign: 'center', cursor: 'pointer' }}>
            <input ref={fileRef} type="file" style={{ display: 'none' }} onChange={e => setFile(e.target.files[0])} />
            {file ? <><CheckCircle2 size={22} color="var(--status-ok)" style={{ margin: '0 auto 6px' }} /><div style={{ fontSize: '12px' }}>{file.name}</div></>
        ...
        {error && <div style={{ marginTop: '8px', fontSize: '12px', color: 'var(--status-err)', background: 'rgba(248,113,113,0.08)', borderRadius: '8px', padding: '8px 12px' }}>{error}</div>}
...
function AssignmentsSection({ assignments, onUpdate }) {
  ...
  const pendingCount = assignments.filter(a => a.submission_status === 'not_submitted').length;
  const doneCount = assignments.filter(a => a.submission_status === 'submitted' || a.submission_status === 'late').length;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '18px' }}>
        {[{ label: 'Total', value: assignments.length, color: 'var(--accent)' }, { label: 'Pending', value: pendingCount, color: 'var(--status-warn)' }, { label: 'Submitted', value: doneCount, color: 'var(--status-ok)' }, { label: 'Overdue', value: overdue, color: 'var(--status-err)' }].map(s => (
      ...
      {(overdue > 0 || dueToday > 0) && (
        <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--status-err)' }}>
      ...
      <div style={{ display: 'flex', gap: '5px', marginBottom: '14px' }}>
        {[['all', 'All'], ['pending', 'Pending'], ['submitted', 'Submitted'], ['overdue', 'Overdue']].map(([k, l]) => (
          <button key={k} onClick={() => setFilter(k)} className={`btn ${filter === k ? 'active' : ''}`} style={{
            background: filter === k ? 'var(--accent-bg)' : 'transparent',
            color: filter === k ? 'var(--accent)' : 'var(--text-muted)',
            borderColor: filter === k ? 'var(--accent-border)' : 'var(--border)'
          }}>{l}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {filtered.length === 0 ? (
          ...
          return (
            <div key={a.id} style={{ background: 'var(--bg-card)', border: `1.5px solid ${due.urgent && canSubmit ? 'rgba(248,113,113,0.3)' : 'var(--border)'}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '7px', flexWrap: 'wrap', marginBottom: '3px' }}>
                    <span style={{ fontSize: '13px', fontWeight: '600' }}>{a.title}</span>
                    <span className="badge-accent">{a.subject}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                    <span style={{ fontSize: '11px', color: due.color, display: 'flex', alignItems: 'center', gap: '3px' }}><Clock size={10} />{due.text}</span>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap ${sc === 'submitted' ? 'badge-green' : sc === 'late' ? 'badge-red' : 'badge-yellow'}`}>{sc === 'not_submitted' ? 'pending' : sc}</span>
                  ...
                  {isSubmitted && (
                    loadingSub === a.id ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--text-muted)' }}>
                        <Loader2 size={12} className="animate-spin" /> Loading submission...
                      </div>
                    ) : sub ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                          ...
                          {canEdit && (
                            <div style={{ display: 'flex', gap: '6px' }}>
                              ...
                              <button className="btn" onClick={() => handleDelete(a)} disabled={deleting === a.id} style={{ color: 'var(--status-err)', borderColor: 'rgba(248,113,113,0.2)' }}>
                        ...
                        {sub.text && (
                          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px', fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{sub.text}</div>
                        )}
                        {sub.file_path && (
                        ...
                        {sub.grade && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: '10px' }}>
                            <Star size={14} className="text-status-ok" />
                            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--status-ok)' }}>Grade: {sub.grade}</span>
                          </div>
                        )}
                        {sub.feedback && (
                          <div style={{ padding: '12px 14px', background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', borderRadius: '10px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                              <MessageSquare size={12} color="var(--accent)" />
                              <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Teacher Feedback</span>
                            </div>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: '24px', right: '24px', background: 'var(--bg-elevated)', border: `1px solid ${toast.type === 'success' ? 'var(--status-ok)' : toast.type === 'warn' ? 'var(--status-warn)' : 'var(--status-err)'}`, borderRadius: '10px', padding: '10px 16px', fontSize: '12px', zIndex: 999, color: 'var(--text-primary)' }}>
          {toast.msg}
        </div>
      )}
    </div>
...
// ─── SECTION: Coding ─────────────────────────────────────────────────────────
const PLATFORMS = [
  { id: 'leetcode',   label: 'LeetCode',   color: '#fbbf24', url: u => `https://leetcode.com/u/${u}` },
  { id: 'github',     label: 'GitHub',     color: '#8b949e', url: u => `https://github.com/${u}` },
  { id: 'codeforces', label: 'Codeforces', color: '#60a5fa', url: u => `https://codeforces.com/profile/${u}` },
...
function VScoreBar({ score }) {
  ...
  const segs = [
    { label: 'LC',  val: bd.lc ?? 0,       max: 350, color: '#fbbf24' },
    { label: 'CF',  val: bd.cf ?? 0,       max: 250, color: '#60a5fa' },
    { label: 'CC',  val: bd.cc ?? 0,       max: 150, color: '#cd7f32' },
    { label: 'GH',  val: bd.gh ?? 0,       max: 150, color: '#8b949e' },
    { label: 'Act', val: bd.activity ?? 0, max: 100, color: 'var(--status-ok)' },
  ];
...
function PlatformMiniCard({ profile, onSync, syncing }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: `1px solid ${meta.color}30`, borderRadius: '12px', padding: '14px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          <button onClick={() => onSync(profile.platform)} disabled={syncing === profile.platform} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '2px', display: 'flex' }}>
            <RefreshCw size={11} className={syncing === profile.platform ? 'animate-spin' : ''} />
          </button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'var(--text-muted)' }}>
        ...
        {profile.estimated_weekly_hours > 0 && <span style={{ color: 'var(--accent)' }}>{safeH(profile.estimated_weekly_hours)}/wk</span>}
      </div>
    </div>
  );
}
...
function ConnectCard({ platformId, onLinked }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.015)', border: '1px dashed var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
      {!open ? (
        ...
        <div style={{ padding: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            ...
            <button className="btn-primary" disabled={busy || !val.trim()} onClick={submit} style={{ fontSize: '11px', height: '32px', padding: '0 10px' }}>
              {busy ? <Loader2 size={11} className="animate-spin" /> : 'Save'}
            </button>
          </div>
          {err && <div style={{ fontSize: '10px', color: 'var(--status-err)', marginTop: '5px' }}>{err}</div>}
...
function LCBreakdown({ p }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
      <div style={{ fontSize: '12px', fontWeight: '600', marginBottom: '12px', color: 'var(--status-warn)' }}>LeetCode Breakdown</div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {board.slice(0, 10).map(row => {
          ...
          return (
            <div key={row.rank} style={{ display: 'grid', gridTemplateColumns: '36px 1fr 90px 70px 70px', gap: '8px', padding: '9px 12px', borderRadius: '8px', background: row.is_me ? `${tier.color}12` : 'rgba(255,255,255,0.02)', border: row.is_me ? `1px solid ${tier.color}40` : '1px solid var(--border)', alignItems: 'center' }}>
...
function CodingSection({ currentUser }) {
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}><Loader2 size={24} className="animate-spin text-accent" /></div>;
    ...
    <div>
      ...
      {summary && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px', marginBottom: '18px' }}>
          {[
            { label: 'Weekly Hours', value: safeH(summary.total_weekly_hours), color: 'var(--accent)' },
            ...
            <div key={label} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
      ...
      {lc && <div style={{ marginBottom: '18px' }}><LCBreakdown p={lc} /></div>}
      {summary?.recent_submissions?.length > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', marginBottom: '18px' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
          <Trophy size={13} color="var(--status-warn)" />
