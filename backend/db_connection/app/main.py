...
from app.auth.router import router as auth_router, ensure_demo_users
from app.auth.rate_limit import purge_old_failed_attempts
...
def _startup() -> None:
    ...
    if not scheduler.running:
        scheduler.add_job(purge_old_failed_attempts, "interval", hours=1, id="purge_failed_logins", replace_existing=True)
        scheduler.start()
